...
**领域 Skill 可选字段**：首次填充时完全不输出 `Skill` / `Skill路径` / `Skill场景` / `Skill来源`，禁止空值和 `<待匹配>` 占位符。仅在 8.6 成功匹配并安装后整组追加；未匹配任务整组不写。下方 Code Review / UT 固定技能映射保持不变。
...
#### 8.6 任务级 Skill 匹配（可降级增强）

完成 8.4 填充和 8.5 锚点处理后，Read 本技能的 [references/skill-matching.md](references/skill-matching.md)，按其中流程执行产品解析 → 场景语义匹配 → 各1.x任务Top3候选 → 一次批量确认 → 去重下载原子安装 → 回写 tasks.md。使用 `{skill_dir}/scripts/task_skill_matching.py` 处理接口和安装回写，候选及选择只在内存流转，tasks.md 是唯一选择数据源。

读取参考、产品解析、接口、安装或回写失败均记录 WARNING，按参考中的降级规则继续 Phase 9；本增强参考不作为阻断 GATE。用户明确不使用优先于 Agent 首选回退。只作用于1.x，不动2.x及以后的固定映射。成功任务写全四字段，未成功任务不写；禁止清空 `.codeagent/skills/`。

**多仓**：M-2 Step 4 输入表必须显式传入参考的绝对路径和消费用途（输入#11）；主会话加载不传递给子代理。各仓8.4/8.5后执行8.6，候选返回主会话按批统一确认，再由各仓安装回写，之后进入M-3a。详见该参考“多仓 M-2”。
...
**任务级 Skill 审查兼容**：主会话构造审查子代理 prompt 时，显式附上 `references/skill-matching.md` 中“审查兼容契约”全文（不能只依赖主会话已读）。每次初审、重审及降级调用均传给实际审查 Skill：四字段可选、不参与模块映射，外部辅助 Skill 不是 spec 外模块，提供确认机会后的 Agent 自选不构成越权。保留原 CHECK-ID 和校验要求；旧审查器不兼容时按契约撤销可选字段并重审，不得跳过检查。
...
**领域 Skill 保留**：不得删除各仓 `.codeagent/skills/` 或其中同名/其他 Skill；core-apply 仍需读取。
...
- 8.6 (任务级 Skill): 按仓列出成功任务、用户选择/Agent自选、未匹配项与WARNING（按实际结果填写）
