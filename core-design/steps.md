...
| 8 | generate-artifacts | Phase 8 | 生成设计文档：读取参考 → 对照原则/视图约束 → 填充模板 → 锚点提取 → 8.6任务级Skill匹配（批量确认、失败降级） |
...
| 8 | M-2 | Phase 8 | 各仓按仓生成设计文档 — 按 multi-repo.md 执行（含 impl_questions 用户确认 + subagent 并行文档生成 + 各仓8.6候选汇总确认及安装回写，替代单仓 generate-artifacts） |
