...
4. **【强制】使用子代理并行生成各仓设计文档**：
   ...
   **子代理类型选择**（按优先级尝试）：
   ...
   | 11 | skill-matching.md | {skill_dir}/references/skill-matching.md | **任务级Skill规则**：8.4/8.5后按本仓产品匹配1.x领域Skill，返回候选供主会话批量确认，安装成功才回写；参考缺失或增强失败只WARNING，不阻断文档生成 |
   ...
   **路径说明**：{skill_dir} 为 core-design 技能安装目录，主会话在构造子代理 prompt 时必须将 {skill_dir} 替换为实际绝对路径（解析方式见 SKILL.md Reference文件路径解析章节）。
   ...
   - 首次生成完全不输出 Skill / Skill路径 / Skill场景 / Skill来源 四字段，不留空或占位符。8.4/8.5完成后读取输入#11，按8.6执行；返回SKILL_CANDIDATES给主会话收齐本批后一次批量确认，再接收按仓结果安装回写。无候选/失败可降级；禁止候选未确认就下载。主会话须在prompt中给出本仓根目录绝对路径。
   - 设计锚点格式规范：
   ...
   **并发规则**（对齐core-explore M-2）：
   ...
   - SKILL_CANDIDATES是等待批量确认的中间状态，不计为失败；主会话按输入#11的交互规则一次确认本批，并恢复各仓安装回写。全部仓完成8.6或明确降级后才输出强制检查点并进入M-3a
   - 所有子代理完成后输出强制检查点
...
1. **使用子代理并行执行各仓L2/L3追溯验证**：
   ...
   任务级Skill兼容规则：主会话必须在此处注入 {skill_dir}/references/skill-matching.md 的“审查兼容契约”全文；子代理每次调用实际审查Skill（含重审）均传入：四字段可选、不参与模块映射；外部辅助Skill不是spec外模块；已提供确认机会的Agent自选不构成越权。不改变原检查集合，旧检查器仅因元数据误判时撤销受影响任务四字段并重审，WARNING记录增强降级。
